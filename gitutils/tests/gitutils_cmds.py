...
class TestGitutils(TestCase):
    ...
    def step1(self):
        ...
        self.assertNotEqual(gitlab_utils.get_group_id(
            self.__class__.group_name), -1)
        time.sleep(2)
    ...
    def step2(self):
        ...
        gitlab_utils.get_project_id(
            self.__class__.group_name, self.__class__.project_1)
        gitlab_utils.get_project_id(
            self.__class__.group_name, self.__class__.project_2)
        time.sleep(2)
    ...
    def step3(self):
        ...
        found = any(proj['name'] ==
                    self.__class__.project_2 for proj in own_projects)
    ...
    def step4(self):
        ...
        # ARG PROJECT
        sys.argv.append(self.__class__.group_name +
                        '/'+self.__class__.project_fork)
        # calls main to fork
        gitutils.main()
        # verifies if forked project exists under group projects
        own_projects = gitlab_utils.get_group_projects(
    ...
    def step5(self):
        ...
        members = gitlab_utils.get_group(gitlab_utils.get_group_id(
            self.__class__.group_name)).members.all(all=True)
        found = False
        ...
        for i in members:
            if i.attributes['name'] == self.__class__.user_ldap_member:
                found = True
                self.assertEqual(i.attributes['access_level'], gitlab_utils.check_role(
                    self.__class__.user_role))
        self.assertTrue(found)
        time.sleep(2)
    ...
    def step8(self):
        ...
        time.sleep(2)
        project_id = gitlab_utils.get_project_id(
            'hax_l', self.__class__.project_2)
